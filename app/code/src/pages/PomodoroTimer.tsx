...
export default function PomodoroTimer() {
  ...
  const { startFocusMode, stopFocusMode } = useLocalUsageTracker();
      ...
      intervalRef.current = setInterval(() => {
        setElapsed((prev) => {
          if (prev + 1 >= totalTime) {
            ...
            setIsRunning(false);
            toast.success("Pomodoro Complete! 3 cycles done!");
            stopFocusMode();
            return totalTime;
          }
          return prev + 1;
  ...
  useEffect(() => {
    ...
    if (lastMode !== mode) {
      ...
      if (mode === "focus") {
        toast.success("Focus Time - Back to work!");
        startFocusMode();
      } else {
        toast.info("Break Time - Take a break!");
        stopFocusMode();
      }
  ...
  const handlePresetSelect = (focus: number, breakT: number) => {
    ...
    setElapsed(0);
    setIsRunning(false);
    stopFocusMode(); // Stop any existing focus tracking
  };
